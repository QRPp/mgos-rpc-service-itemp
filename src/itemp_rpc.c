...
#define SEND_CMD_FMT "{src:%H,cmd:%u,arg:%f}"
static void itemp_send_cmd_handler(struct mg_rpc_request_info *ri, void *cb_arg,
  ...
  if (!src || cmd == INT_MAX) mg_rpc_errorf_ret(400, "src&cmd are required");
  if (src_sz != 3) mg_rpc_errorf_gt(400, "src must be 3 bytes");
  if (!mgos_itemp_send_cmd(src[0] << 16 | src[1] << 8 | src[2], cmd, arg / 0.5))
